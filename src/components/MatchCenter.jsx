...
export default function MatchCenter({
  ...
  async function handleUndoLastEvent() {
    ...
    if (lastEvent.type === "penalty_shootout_goal") {
      if (isHomeEvent) {
        newHomePen = Math.max(0, newHomePen - 1);
      } else {
        newAwayPen = Math.max(0, newAwayPen - 1);
      }
    } else if (EVENT_TYPES[lastEvent.type]?.scores === true) {
      if (isHomeEvent) {
    ...
    if (EVENT_TYPES[lastEvent.type]?.countsGoal === true) {
      rebuildGoalScorers(updatedFixtures);
    }
  }
  ...
  async function handleFinishMatch() {
    ...
    if (!liveMatch.isKnockout && Number.isFinite(safeId)) {
      const { error } = await supabase
        .from("fixtures")
        .update({
          home_score: liveMatch.homeScore,
          away_score: liveMatch.awayScore,
          played: true,
        })
        .eq("id", safeId);
